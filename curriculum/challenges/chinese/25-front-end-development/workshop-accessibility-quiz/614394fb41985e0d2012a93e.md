...
问题编号的描述性不够。对于视障用户尤其如此。解决此类问题的一种方法是，在不向元素添加可见文本的情况下，添加只有屏幕阅读器可以阅读的文本。

在每个 `h3` 元素中的数字后面嵌套一个 `class` 为 `sr-only` 的 `span` 元素。
...
你应该给第一个 `span` 元素一个 `sr-only` 的 `class`。
...
你应该给第二个 `span` 元素一个 `sr-only` 的 `class`。
