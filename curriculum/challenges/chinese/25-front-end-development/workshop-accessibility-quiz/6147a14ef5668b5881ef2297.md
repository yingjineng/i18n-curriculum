...
为了使导航按钮看起来更像典型的按钮，请删除锚标签的下划线。

然后，创建一个新的选择器，以导航列表元素为目标，以便当光标悬停在它们上面时，背景颜色和文本颜色会切换，并且光标会变成指针。
...
你应该使用 `nav > ul > li:hover` 或 `nav li:hover` 选择器来设置元素悬停时的样式。
...
你应该给悬停的 `li` 元素设置 `#dfdfe2` 的 `background-color`。
...
你应该给悬停的 `li` 元素设置 `#1b1b32` 的 `color`。
...
你应该给悬停的 `li` 元素设置 `pointer` 的 `cursor`。
