...
在小屏幕上，导航栏中的无序列表会溢出屏幕右侧。

通过使用 _Flexbox_ 来包裹 `ul` 内容来解决此问题。然后，设置以下 CSS 属性以正确对齐文本：
