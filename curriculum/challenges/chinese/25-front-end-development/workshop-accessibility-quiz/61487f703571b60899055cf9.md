...
将 `section` 元素的宽度设置为其父容器的 `80%`。然后，使用外边距将 `section` 元素居中，并为底部外边距添加 `10px`。

此外，确保 `section` 元素的宽度不能超过 `600px`。
