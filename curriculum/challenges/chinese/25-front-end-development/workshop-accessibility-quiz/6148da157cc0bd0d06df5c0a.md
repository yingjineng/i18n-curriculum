...
要使输入框彼此对齐，请创建一个新规则集，该规则集针对 `.info` 元素中的所有 `input` 和 `label` 元素，并将 `display` 属性设置为 `inline-block`。
...
你应该给 `input` 和 `label` 元素设置 `display` 为 `inline-block`。
...
你应该给 `label` 元素设置 `text-align` 为 `right`。
