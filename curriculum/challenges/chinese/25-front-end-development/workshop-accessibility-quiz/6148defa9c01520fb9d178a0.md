...
虽然 `ul`/`li` 元素非常适合为列表项提供项目符号，但你的单选按钮不需要它们。 你可以使用 `list-style` 属性控制项目符号的外观。 例如，你可以使用以下代码将项目符号变成圆形：
...
通过将其样式设置为 `none` 来删除 `.answers-list` 项的默认样式，并删除无序列表的内边距。
