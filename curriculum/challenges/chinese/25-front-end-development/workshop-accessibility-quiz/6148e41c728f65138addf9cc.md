...
最后，某些类型的运动动画可能会给一些用户带来不适。特别是，患有<dfn>前庭</dfn>疾病的人对某些运动触发因素很敏感。

`@media` at-rule 有一个名为 `prefers-reduced-motion` 的媒体特性，可以根据用户的动画偏好设置 CSS。它可以采用以下值之一：
...
你将在未来的课程中学习更多关于此特性的内容。现在，将设置 `scroll-behavior: smooth` 的样式规则包装在 `@media` at-rule 中，并将媒体特性 `prefers-reduced-motion` 的值设置为 `no-preference`。
