...
虽然对于嵌套了 `input` 的 `label` 元素来说，这不是必需的，但明确地将 `label` 与其对应的 `input` 元素链接起来仍然是最佳实践。

现在，为你的四个 `label` 各添加一个 `for` 属性，将 `label` 链接到其对应的单选 `input`。
...
你应该给第一个 `label` 一个 `for` 属性，使其与 `input` 元素的 `id` 相匹配。
...
你应该给第二个 `label` 一个 `for` 属性，使其与 `input` 元素的 `id` 相匹配。
...
你应该给第三个 `label` 一个 `for` 属性，使其与 `input` 元素的 `id` 相匹配。
...
你应该给第四个 `label` 一个 `for` 属性，使其与 `input` 元素的 `id` 相匹配。
