...
在第一个 `tr` 中，添加一个 `th` 元素，其文本为 `Loans The outstanding balance on our startup loan.`。将除 `Loans` 之外的文本包装在 `span` 元素中，并将 `class` 设置为 `description`。

在其下方添加三个 `td` 元素，并按顺序为其指定以下文本：`$500`、`$250` 和 `$0`。为第三个 `td` 元素指定 `class` 为 `current`。
...
你应该将文本 `The outstanding balance on our startup loan.` 包装在 `span` 元素中。
