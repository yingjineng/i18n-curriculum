...
使用 `h1 .flex` 选择器定位你的 flex 容器。将其 `display` 属性设置为 `flex` 以启用 flexbox 布局。然后将 `flex-direction` 属性设置为 `column-reverse` - 这将使嵌套元素从下到上显示。最后，将 `gap` 属性设置为 `1rem` 以在元素之间创建一些空间。
